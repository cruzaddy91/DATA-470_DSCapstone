...
\centering
\includereportfig{../output/figures/target_balance_v2_ordertime.png}
\captionof{figure}{\textbf{Class balance of the v2 order-time target.} Roughly 3.4 percent of order lines carry confirmed backorder risk; 57.8 percent are confirmed-not-at-risk; the remaining 40.2 percent are unresolved at the time of label snapshot and are excluded from training. The right panel shows the parallel overstock target (not the focus of this paper).}
...
\bottomrule
\end{tabular}
\captionof{table}{Tuned configurations for the stack's tunable base learners. Inner PR-AUC is the mean across three temporal expanding-window folds on the training set; the outer holdout is never used for tuning. Source path is in the running text above.}
\label{tbl-hparams}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includereportfig{../output/figures/temporal_positive_rate_drift_v2_ordertime.png}
\captionof{figure}{\textbf{Monthly positive rate by order month.} The pre-cutoff bursts in late-2020 and early-2021 reflect a window where labels matured fully; the test tail (post 2021-07-22) sits below the long-run mean (dotted) because labels for those months have not all resolved yet. This is the visual version of the label-maturity gate that drives the deployment NO-GO discussed later.}
\label{fig-drift}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includegraphics[width=\linewidth,height=0.34\textheight,keepaspectratio]{../output/figures/roc_curves_temporal_v2_ordertime.png}\\[0.5em]
\includegraphics[width=\linewidth,height=0.34\textheight,keepaspectratio]{../output/figures/pr_curves_temporal_v2_ordertime.png}
\captionof{figure}{\textbf{Discrimination on the temporal holdout.} \emph{Top:} ROC curves; the stack reaches AUC 0.94 and logistic regression 0.91, with tree-only baselines trailing. \emph{Bottom:} precision--recall curves; logistic regression leads under prevalence-aware ranking with PR-AUC 0.36, the stack 0.33.}
\label{fig-roc-pr}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includereportfig{../output/figures/classification_confusion_matrices_v2_ordertime.png}
\captionof{figure}{\textbf{Row-normalized confusion matrices on the temporal holdout.} Logistic regression catches 59 percent of true positives at threshold $\tau = 0.364$; the stack catches 43 percent at a higher cutoff $\tau = 0.660$. Both keep false-positive rates near 1 percent on the actual-negative row.}
\label{fig-confusion}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includereportfigtall{../output/figures/business_utility_curves.png}
\captionof{figure}{\textbf{Business-utility curves: expected cost per row vs decision threshold} across asymmetric cost ratios $k = c_{\mathrm{miss}}/c_{\mathrm{false\ alarm}}$. The recommended operating point (filled dot) is robust for $k \in [3, 30]$.}
\label{fig-utility}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includereportfig{../output/figures/evidence_lift_gains_v2_ordertime.png}
\captionof{figure}{\textbf{Cumulative gains and lift on the temporal holdout.} Left: cumulative gains; the top 5 percent of scored lines contains roughly 80 percent of the period's confirmed backorders. Right: lift curve; peak lift exceeds 100x at the head of the score distribution and decays smoothly as the review queue grows.}
\label{fig-lift}
\end{minipage}
\end{strip}
\FloatBarrier
...
\noindent\begin{minipage}{\textwidth}
\centering
\includereportfigtall{../output/figures/classification_feature_importance_logistic_regression_v2_ordertime.png}
\captionof{figure}{\textbf{Top features driving logistic regression} on the order-time table. Historical positive-rate features at the customer, material, and client-by-material grain dominate; item-category and country indicators round out the top of the ranking. All features here respect the leakage policy.}
...
\bottomrule
\end{tabular}
\captionof{table}{Negative-results log from the experiment branch. Full method, decision gates, and artifact paths per experiment are in the project's performance experiment report. All four outcomes triangulate the same finding: the feature set, not the modeling layer, is what bounds performance on this dataset.}
\label{tbl-negatives}
\end{minipage}
\end{strip}
\FloatBarrier
...
\bottomrule
\end{tabular}
\captionof{table}{Deployment gate decomposition for the rule-selected champion on the outer temporal holdout. The model clears its operating floors; the dataset does not yet clear the maturity floors. Source: deployment-readiness keys in the metrics JSON.}
\label{tbl-gates}
\end{minipage}
\end{strip}
\FloatBarrier
