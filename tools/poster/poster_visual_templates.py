...
FIG_FILES = {
    ...
    "deploy_summary": "deployment_summary_table_v2_ordertime.png",
}
...
def render_target_balance(spec: dict, root: Path) -> Path | None:
    ...
    axes[0].set_title("Backorder rate (order time v2)")
    ...
    if ospec and ospec.get("counts"):
        ...
        axes[1].set_title("Overstock rate (material/plant)")
...
def render_model_comparison_heatmap(spec: dict, root: Path) -> Path | None:
    ...
    fig.suptitle(
        "Logistic vs. XGBoost vs. Stack: ranking metrics under stress splits",
        fontsize=20,
        fontweight="bold",
        y=0.99,
    )
    ...
    fig.text(
        0.5,
        0.10,
        "Scope: order time v2 table scores. Extension: staged T minus k pre outcome model; see poster.",
        ha="center",
        va="top",
        fontsize=11.5,
        color=FLINT,
    ...
    fig.text(
        ...
        "Night outline = best in column. Poster trio: LR, XGBoost, Stack.",
        ha="center",
        va="bottom",
        fontsize=12,
        color=FLINT,
...
def render_temporal_figures(spec: dict, root: Path) -> dict[str, Path]:
    ...
    for name, color, label_short in (
        ...
        ("oof_calibrated_stack", THISTLE, "Stack (selected)"),
    ...
    ax.set_title("ROC: temporal holdout")
    ...
    # PR: landscape poster panel so the curves and legend remain readable when
    # stacked full-width in the validation column.
    ...
    for name, color, label_short in (
        ...
        ax.plot(rec, prec, color=color, linewidth=3.0, label=f"{label_short} (PR AUC={pr_auc:.2f})")
    ax.set_xlabel("Recall")
    ax.set_ylabel("Precision")
    ax.set_title("Precision recall: temporal holdout")
    ...
    if rel_diag:
        ...
        if diag_path.is_file():
            ...
            if monthly:
                ...
                ax.set_title("Temporal drift: positive rate by month")
                ax.axhline(
                    baseline_rate * 100.0,
                    color=COLOR_BASELINE,
                    linestyle=":",
    ...
    for label_short, key in (("Logistic Regression", "logistic_regression"), ("Stack (selected)", "oof_calibrated_stack")):
        if key in models:
    ...
    if model_items:
        ...
        for ax, (name, label_short) in zip(axes.flat, model_items):
            ...
            ax.set_title(f"{label_short}: score distribution")
    ...
    # Confusion matrices (row-normalized), side by side. Headline pair on the
    # current poster: Logistic Regression (deployable best on outer temporal)
    # and the OOF-calibrated Stack (rule-selected champion per protocol §9).
    cmap_cm = brand_confusion_heatmap_cmap()
    fig, axes = plt.subplots(1, 2, figsize=(15, 5.8))
    for ax, (name, label_short) in zip(axes, (("logistic_regression", "Logistic Regression"), ("oof_calibrated_stack", "Stack (selected)"))):
        if name not in models:
            ax.axis("off")
            continue
    ...
    plt.suptitle("Confusion matrices: temporal holdout", y=1.01, fontsize=16, fontweight="bold")
    fig.tight_layout()
...
def render_deployment_summary_table(spec: dict, root: Path) -> Path | None:
    """Brand-styled deployment-readiness summary table.
    Two headline models (LR + Stack), their outer-temporal metrics, and the
    project's honest deploy-gate verdict. Replaces the bottom-right validation
    flow mermaid in the poster pipeline. Designed as a landscape panel for the
    stacked validation column.
    """
    rel = (spec.get("truth_paths") or {}).get("classification_model_comparison_csv")
    if not rel:
        return None
    ...
    if not csv_path.is_file():
        return None
    df = pd.read_csv(csv_path)
    th = df[df["split"] == "temporal_holdout"].set_index("model")

    # Pull both headline rows. Tolerate missing models.
    def _row(name: str):
        if name not in th.index:
            return None
        r = th.loc[name]
        return {
            "label": "Logistic Regression" if name == "logistic_regression" else "Stack (selected)",
            "roc": float(r.get("roc_auc", float("nan"))),
            "pr": float(r.get("pr_auc", float("nan"))),
            "p": float(r.get("precision", float("nan"))),
            "r": float(r.get("recall", float("nan"))),
            "f1": float(r.get("f1", float("nan"))),
            "thr": float(r.get("decision_threshold", float("nan"))),
        }

    lr = _row("logistic_regression")
    stack = _row("oof_calibrated_stack")
    if lr is None or stack is None:
        return None
    ...
    header = patches.FancyBboxPatch(
        (0.04, 0.88), 0.92, 0.10,
        boxstyle="round,pad=0.012,rounding_size=0.02",
        linewidth=0, facecolor=TINT_HEADER, transform=ax.transAxes,
    )
    ax.add_patch(header)
    ax.text(0.5, 0.945, "Deployment readiness summary",
            ha="center", va="center", fontsize=22, fontweight="bold",
            color=NIGHT, transform=ax.transAxes)
    ax.text(0.5, 0.905, "Outer temporal holdout: LR (deployable best) vs. Stack (rule selected champion)",
            ha="center", va="center", fontsize=11, color=FLINT, transform=ax.transAxes)

    # Column headers
    cols = ["Model", "ROC", "PR", "P", "R", "F1", "Threshold"]
    col_xs = [0.06, 0.30, 0.40, 0.50, 0.60, 0.70, 0.82]
    col_widths = [0.24, 0.10, 0.10, 0.10, 0.10, 0.10, 0.14]
    header_y = 0.78
    for x, w, label in zip(col_xs, col_widths, cols):
        ...
        ax.add_patch(rect)
        ax.text(x + w / 2, header_y + 0.035, label,
                ha="center", va="center", fontsize=12, fontweight="bold",
                color=NIGHT, transform=ax.transAxes)

    # Two model rows
    row_specs = [
        (0.66, lr, TINT_CARD_A, NIGHT),
        (0.55, stack, TINT_CARD_B, COPPER),
    ]
    for ry, mdl, fill, accent in row_specs:
        # Row background
        bg = patches.FancyBboxPatch(
            (0.05, ry), 0.92, 0.09,
            boxstyle="round,pad=0.008,rounding_size=0.012",
            linewidth=1.0, edgecolor=EDGE_SUBTLE, facecolor=fill,
            transform=ax.transAxes,
        )
        ax.add_patch(bg)
        # Model label
        ax.text(col_xs[0] + 0.012, ry + 0.045, mdl["label"],
                ha="left", va="center", fontsize=13, fontweight="bold",
                color=accent, transform=ax.transAxes)
        # Cells
        cells = [mdl["roc"], mdl["pr"], mdl["p"], mdl["r"], mdl["f1"], mdl["thr"]]
        for x, w, val in zip(col_xs[1:], col_widths[1:], cells):
            ax.text(x + w / 2, ry + 0.045,
                    "NA" if (val != val) else f"{val:.3f}",
                    ha="center", va="center", fontsize=15,
                    color=FLINT, transform=ax.transAxes)

    # Gate verdict block
    verdict_y = 0.36
    verdict = patches.FancyBboxPatch(
        (0.05, verdict_y), 0.92, 0.16,
        boxstyle="round,pad=0.012,rounding_size=0.02",
        linewidth=1.5, edgecolor=NIGHT, facecolor=TINT_CARD_C,
        transform=ax.transAxes,
    )
    ax.add_patch(verdict)
    ax.text(0.07, verdict_y + 0.115, "Deploy gate: precision floor 0.15, recall floor 0.35",
            ha="left", va="center", fontsize=13, fontweight="bold",
            color=NIGHT, transform=ax.transAxes)

    p_pass = stack["p"] >= 0.15
    r_pass = stack["r"] >= 0.35
    p_mark = "PASS" if p_pass else "FAIL"
    r_mark = "PASS" if r_pass else "FAIL"
    ax.text(0.07, verdict_y + 0.06,
            f"Stack precision {stack['p']:.3f}: {p_mark}        Stack recall {stack['r']:.3f}: {r_mark}",
            ha="left", va="center", fontsize=12, color=FLINT, transform=ax.transAxes)
    ax.text(0.07, verdict_y + 0.020,
            f"LR precision {lr['p']:.3f}: PASS        LR recall {lr['r']:.3f}: PASS",
            ha="left", va="center", fontsize=12, color=FLINT, transform=ax.transAxes)

    # Label maturity verdict. Uses COPPER (brand-approved warning accent) and
    # TINT_CARD_C (Copper tint) so the FAIL panel reads as "alert" while
    # staying strictly in the Westminster palette.
    mat_y = 0.18
    mat = patches.FancyBboxPatch(
        (0.05, mat_y), 0.92, 0.16,
        boxstyle="round,pad=0.012,rounding_size=0.02",
        linewidth=1.5, edgecolor=COPPER, facecolor=TINT_CARD_C,
        transform=ax.transAxes,
    )
    ax.add_patch(mat)
    ax.text(0.07, mat_y + 0.115, "Label maturity gate (binding): FAIL",
            ha="left", va="center", fontsize=13, fontweight="bold",
            color=COPPER, transform=ax.transAxes)
    ax.text(0.07, mat_y + 0.06,
            "Last 180 days: 36% label coverage (min 50%); 32 positives (min 40).",
            ha="left", va="center", fontsize=12, color=FLINT, transform=ax.transAxes)
    ax.text(0.07, mat_y + 0.020,
            "Dataset condition, not a model defect. Both model gates pass.",
            ha="left", va="center", fontsize=12, fontweight="bold", color=FLINT, transform=ax.transAxes)

    # Bottom note
    ax.text(0.5, 0.06,
            "Numbers from output/tables/classification_model_comparison_v2_ordertime.csv",
            ha="center", va="center", fontsize=10, color=FLINT, fontstyle="italic",
            transform=ax.transAxes)

    out = _figures_dir(root) / FIG_FILES["deploy_summary"]
    fig.savefig(out, dpi=200, bbox_inches="tight", facecolor="white")
    plt.close(fig)
    return out
...
def render_all_from_spec(spec: dict, root: Path) -> dict[str, Path | None]:
    ...
    results["deploy_summary"] = render_deployment_summary_table(spec, root)
    return results
